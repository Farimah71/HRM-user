...
export const PositionCard = ({ positions }) => {
  ...
  return positions?.map((position) => (
    <div className="w-full bg-white rounded-11 p-6 flex justify-between gap-x-8" key={position.id}>
      <div className="flex items-center gap-x-8">
        <div className="w-52 h-52 border-2 border-custom-gray-light rounded-30 overflow-hidden">
          <img src={position.logo} alt="logo" className="w-full"/>
        </div>
        <div className="w-max h-full flex flex-col justify-between items-start">
          <h4 className="text-custom-orange text-19">{position.company}</h4>
          <p className="text-19 font-bold">{position.title}</p>
          <div className="flex items-center gap-x-8">
            <p className="text-14">{position.features[0]}</p>
            <p className="text-14">{position.features[1]}</p>
            <p className="text-14 text-custom-gray-muted">{position.salary}</p>
          </div>
          <div className="flex items-center gap-x-2 rounded-48 bg-custom-gray-light p-2">
            ...
            <span className="text-14">{position.location}</span>
          </div>
          <Link to="/job-detail" className="h-10 flex items-center justify-center bg-custom-orange uppercase w-60 rounded-md text-16 text-white">{t("button.more_title")}</Link>
        </div>
      </div>
      <div className="w-max flex flex-col items-center justify-between">
        ...
        <p className="text-custom-dark text-opacity-30">2 {t("positions.hours_ago")}</p>
      </div>
    </div>
  ));
};
